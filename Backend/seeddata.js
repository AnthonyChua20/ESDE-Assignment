            ...
            try {
                ...
                try {
                    ...
                    if (createFileDataResult) {
                        logger.info(`rita_design${index}.png is uploaded. 1 record created in file table.`)
                    }
                } catch (error) {
                    logger.info(`rita_design${index}.png is is not uploaded. Did not create 1 record in file table.`)
                    logger.info(error);
                }
            } catch (error) {
                logger.info('\n\nuploadFileToCloudinary has error\n', error);

            } //End of try .. catch for calling uploadFileToCloudinary
        } //End of for loop block

    } catch (error) {
        logger.info('\n\nCreate rita user record has error\n', error);
        return;
    } finally {
        logger.info('\n\nData seeding of rita record in user table, rita file record in file table and image files in cloud has completed.');
        return;
    }
...
async function prepareAdminRobertData() {
    try {
        ...
        logger.info('\n\nCreate robert user record has error\n', error);
        return;
    } finally {
        logger.info('\n\nData seeding of robert record in user table has completed.');
        return;
    }
